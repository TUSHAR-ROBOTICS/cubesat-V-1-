/*
  ...
  --------------------------------------------------------
  Sensors combined on one I2C bus:
    - BMP280   (temperature, pressure, altitude)   -> Adafruit_BMP280 library
    - INA219   (voltage, current, power)           -> Adafruit_INA219 library
    - ICM-20948(accel, gyro)                       -> raw register access, NO library
  ...
  WIRING:
    SDA -> GPIO 8
    SCL -> GPIO 9
    All three sensors share this same bus.
...
:root{
  ...
  --text:#fff;--dim:#888;--dim2:#444;
  --mono:'Share Tech Mono',monospace;
}
html,body{height:100%;overflow:hidden;}
body{background:var(--bg);color:var(--text);font-family:var(--mono);display:flex;flex-direction:column;font-size:10px;}
.topbar{height:30px;display:flex;align-items:center;justify-content:space-between;padding:0 14px;border-bottom:1px solid var(--border);background:var(--bg2);flex-shrink:0;}
.tb-left{display:flex;align-items:center;gap:12px;}
.tb-title{font-size:12px;letter-spacing:3px;color:#fff;}
.tb-sep{width:1px;height:12px;background:var(--border);}
.tb-tag{font-size:8px;letter-spacing:2px;color:var(--dim);}
.tb-right{display:flex;align-items:center;gap:10px;}
.live-dot{width:5px;height:5px;border-radius:50%;background:#fff;animation:blink 1.8s infinite;}
@keyframes blink{0%,100%{opacity:1;}50%{opacity:0.2;}}
.clock{font-size:10px;letter-spacing:2px;color:#fff;}

.body{display:flex;flex:1;overflow:hidden;min-height:0;}

/* ---- LEFT: small 3D viewport + camera, stacked ---- */
.left{width:270px;flex-shrink:0;display:flex;flex-direction:column;border-right:1px solid var(--border);overflow:hidden;}
.mini-panel-label{font-size:7px;letter-spacing:2px;color:var(--dim2);padding:3px 8px;border-bottom:1px solid var(--border2);flex-shrink:0;text-transform:uppercase;display:flex;justify-content:space-between;}
#canvasContainer{flex:1;position:relative;min-height:0;border-bottom:1px solid var(--border);background:#000;}
#canvasContainer canvas{display:block;width:100%!important;height:100%!important;}
.hud-mini{position:absolute;bottom:4px;right:6px;font-size:7px;letter-spacing:1px;color:rgba(255,255,255,0.35);text-align:right;line-height:1.5;pointer-events:none;}
.cam-box{flex:1;display:flex;flex-direction:column;min-height:0;background:#000;}
...
#cam-msg{font-size:8px;color:#333;letter-spacing:2px;}
.corner{position:absolute;width:10px;height:10px;pointer-events:none;border-color:rgba(255,255,255,0.15);}
.c-tl{top:4px;left:4px;border-top:1px solid;border-left:1px solid;}
.c-tr{top:4px;right:4px;border-top:1px solid;border-right:1px solid;}
.c-bl{bottom:4px;left:4px;border-bottom:1px solid;border-left:1px solid;}
.c-br{bottom:4px;right:4px;border-bottom:1px solid;border-right:1px solid;}
#cam-live-dot{display:none;position:absolute;top:4px;left:50%;transform:translateX(-50%);font-size:7px;color:rgba(255,255,255,0.35);letter-spacing:1px;}

/* ---- RIGHT: dense no-scroll data grid ---- */
.right{flex:1;display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:1fr 1fr 100px;overflow:hidden;min-height:0;}
...
.right>.card:nth-child(3n){border-right:none;}
.card-h{font-size:7px;letter-spacing:2px;color:var(--dim2);text-transform:uppercase;padding:4px 10px;border-bottom:1px solid var(--border2);flex-shrink:0;display:flex;justify-content:space-between;}
.card-body{flex:1;padding:4px 10px;overflow:hidden;min-height:0;display:flex;flex-direction:column;justify-content:center;gap:2px;}
.row{display:flex;justify-content:space-between;align-items:center;padding:2px 0;}
.rk{font-size:9px;color:var(--dim);letter-spacing:0.5px;}
.rv{font-size:12px;color:#fff;letter-spacing:0.5px;}
.rv.dim{color:var(--dim);font-size:10px;}
.rv.warn{color:#aaa;}
.stat-trio{display:flex;justify-content:space-between;gap:6px;padding:2px 0 6px;}
.stat-item{flex:1;text-align:center;border-right:1px solid var(--border2);}
.stat-item:last-child{border-right:none;}
.stat-num{font-size:20px;color:#fff;line-height:1;}
.stat-unit{font-size:7px;color:var(--dim2);letter-spacing:1px;margin-top:2px;}
.chart-wrap{height:44px;position:relative;margin-top:2px;}
...
.ring-lbl{font-size:6.5px;letter-spacing:1.5px;color:var(--dim2);}

.console-card{grid-column:1/4;border-right:none;}
.console-body{flex:1;overflow-y:auto;font-size:8.5px;color:var(--dim);line-height:1.5;white-space:pre-wrap;word-break:break-all;padding:3px 10px;min-height:0;}
.console-body::-webkit-scrollbar{width:2px;}
...
.statusbar{height:18px;display:flex;align-items:center;justify-content:space-between;padding:0 14px;border-top:1px solid var(--border);background:var(--bg2);flex-shrink:0;}
.sb-item{font-size:7.5px;letter-spacing:1px;color:var(--dim2);display:flex;align-items:center;gap:4px;}
.sb-dot{width:4px;height:4px;border-radius:50%;background:#fff;}
...
<div class="topbar">
  <div class="tb-left">
    <span class="tb-title">CUBESAT//OS</span>
...
<div class="body">
  ...
  <div class="left">
    <div class="mini-panel-label">3D ATTITUDE <span id="fps-hud">60 FPS</span></div>
    <div id="canvasContainer">
      <div class="hud-mini" id="rot-hud">P: 0.0 R: 0.0</div>
    </div>
    <div class="mini-panel-label">PI CAMERA <span id="cam-status">CONNECTING...</span></div>
    <div class="cam-box">
      <div class="cam-view">
        <img id="camFeed" alt=""/>
  ...
  <div class="right">
    ...
    <div class="card">
      <div class="card-h">BARO · BMP280</div>
      <div class="card-body">
        <div class="stat-trio">
    ...
    <div class="card">
      <div class="card-h">ACCELEROMETER · ICM</div>
      <div class="card-body">
        ...
        <div class="row"><span class="rk">VIB RMS</span><span class="rv dim" id="vib">--</span></div>
        <div class="chart-wrap"><canvas id="accelChart"></canvas></div>
      </div>
    </div>

    <div class="card">
      <div class="card-h">ATTITUDE</div>
      <div class="card-body">
        <div class="rings">
          <div class="ring-item">
            <svg width="42" height="42" viewBox="0 0 48 48">
              <circle cx="24" cy="24" r="19" stroke="rgba(255,255,255,0.08)" stroke-width="2" fill="none"/>
              <circle cx="24" cy="24" r="19" stroke="#fff" stroke-width="1.5" fill="none" stroke-dasharray="119" id="r-roll" stroke-dashoffset="119" stroke-linecap="round" transform="rotate(-90 24 24)"/>
              <text x="24" y="28" text-anchor="middle" font-family="Share Tech Mono,monospace" font-size="8" fill="#fff" id="rv-roll">0°</text>
            </svg>
          ...
          <div class="ring-item">
            <svg width="42" height="42" viewBox="0 0 48 48">
              <circle cx="24" cy="24" r="19" stroke="rgba(255,255,255,0.08)" stroke-width="2" fill="none"/>
              <circle cx="24" cy="24" r="19" stroke="#aaa" stroke-width="1.5" fill="none" stroke-dasharray="119" id="r-pitch" stroke-dashoffset="119" stroke-linecap="round" transform="rotate(-90 24 24)"/>
              <text x="24" y="28" text-anchor="middle" font-family="Share Tech Mono,monospace" font-size="8" fill="#aaa" id="rv-pitch">0°</text>
            </svg>
          ...
          <div class="ring-item">
            <svg width="42" height="42" viewBox="0 0 48 48">
              <circle cx="24" cy="24" r="19" stroke="rgba(255,255,255,0.08)" stroke-width="2" fill="none"/>
              <circle cx="24" cy="24" r="19" stroke="#555" stroke-width="1.5" fill="none" stroke-dasharray="119" id="r-yaw" stroke-dashoffset="119" stroke-linecap="round" transform="rotate(-90 24 24)"/>
              <text x="24" y="28" text-anchor="middle" font-family="Share Tech Mono,monospace" font-size="8" fill="#666" id="rv-yaw">0°</text>
            </svg>
    ...
    <div class="card">
      <div class="card-h">GYROSCOPE · ICM</div>
      <div class="card-body">
    ...
    <div class="card">
      <div class="card-h">POWER · INA219</div>
      <div class="card-body">
        ...
        <div class="row"><span class="rk">STATE</span><span class="rv" id="bstate">--</span></div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="row"><span class="rk">WIFI RSSI</span><span class="rv" id="rssi">--</span></div>
        <div class="row"><span class="rk">CPU TEMP</span><span class="rv warn" id="cputemp">--</span></div>
        <div class="row"><span class="rk">UPTIME</span><span class="rv" id="upt">--</span></div>
        <div class="row"><span class="rk">ERRORS / RECON</span><span class="rv dim" id="errsline">--</span></div>
      </div>
    </div>

    <div class="card console-card">
      <div class="card-h">CONSOLE <span>LIVE</span></div>
      <div class="console-body" id="con"><span class="log-w">// CUBESAT_OS v1.1 (real telemetry, single-view)</span>
...
<div class="statusbar">
  ...
  <div class="sb-item"><div class="sb-dot off" id="dot-baro"></div>BARO</div>
  ...
  <div class="sb-item" style="color:#444">v1.1 // single-view build</div>
</div>
...
// ============ THREE.JS SCENE (small viewport, real-data driven rotation) ============
...
const acChart=new Chart(acx,{
  ...
  data:{
    ...
    datasets:[
      ...
      {data:Array(N).fill(0),borderColor:'#666',borderWidth:1,fill:false,tension:0.3,pointRadius:0},
      {data:Array(N).fill(1),borderColor:'#333',borderWidth:1,fill:false,tension:0.3,pointRadius:0}
    ]
  },
  options:{
...
function updateUI(d) {
  ...
  setRing('r-roll', 'rv-roll', d.roll, 90);
  setRing('r-pitch', 'rv-pitch', d.pitch, 90);
  setYawRing(d.yaw);
  document.getElementById('rot-hud').textContent = `P: ${d.pitch.toFixed(1)} R: ${d.roll.toFixed(1)}`;

  targetPitch = d.pitch * Math.PI / 180;
  targetRoll  = d.roll  * Math.PI / 180;
  targetYaw   = d.yaw   * Math.PI / 180;
  ...
  document.getElementById('bstate').textContent = d.ina_voltage > 4.5 ? 'NOMINAL' : 'LOW';
...
// ============ PI CAMERA FEED ============
const CAM_BASE = 'http://192.168.137.53:5000';
